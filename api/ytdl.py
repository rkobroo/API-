...
async def get_info(q: str, f: str = DEFAULT_FORMAT):
    ydl_opts = {
        ...
        "retries": 3,
        "encoding": "utf8",
        "socket_timeout": 10,  # Add timeout in seconds
    }
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e), headers={"Cache-Control": "no-store, max-age=0"})
        except Exception as e:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e), headers={"Cache-Control": "no-store, max-age=0"})
